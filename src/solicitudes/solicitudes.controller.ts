...
export class SolicitudesController {
  ...
  @ApiResponse({
    status: 403,
    description: 'Solo clientes pueden crear solicitudes',
  })
  async create(
  ...
  @ApiOperation({
    ...
    description:
      'Sube la foto a Storage y devuelve el path para incluir en fotos_urls al crear la solicitud.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async uploadFotoProblema(
    ...
    return this.solicitudesService.uploadFotoProblema(
      user.id,
      file,
      accessToken,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Clientes ven sus propias solicitudes. Prestadores verificados ven trabajos disponibles por rubro y radio.',
  })
  async findAll(@CurrentUser() user: User, @AccessToken() accessToken: string) {
    return this.solicitudesService.findAll(user.id, accessToken);
  }

  @Get('my-active')
  @ApiOperation({
    summary: 'Trabajo activo del prestador (aceptado/finalizado)',
  })
  async getMyActive(
    @CurrentUser() user: User,
    @AccessToken() accessToken: string,
  ...
  @Get('history')
  @ApiOperation({
    summary: 'Historial de trabajos del prestador (finalizado/cerrado)',
  })
  async getHistory(
    @CurrentUser() user: User,
    @AccessToken() accessToken: string,
  ...
  @ApiOperation({
    summary: 'Postulaciones activas del prestador en solicitudes programadas',
  })
  ...
  @ApiOperation({
    ...
    description:
      'El primer prestador que acepta se lleva el trabajo. Race-condition seguro.',
  })
  @ApiResponse({ status: 200 })
  @ApiResponse({
    status: 409,
    description: 'Trabajo ya tomado por otro técnico',
  })
  @ApiResponse({ status: 403 })
  async accept(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary:
      'Postularse como candidato (solo solicitudes en modo programado, hasta 3 cupos)',
    description:
      'El precio no se cotiza acá: se acuerda por chat con el cliente una vez postulado.',
  })
  @ApiResponse({ status: 200 })
  @ApiResponse({
    status: 409,
    description: 'Cupo completo, ya postulado, o plazo vencido',
  })
  @ApiResponse({ status: 403 })
  async postularse(
    @Param('id') id: string,
    ...
    return this.solicitudesService.postularse(
      id,
      user.id,
      accessToken,
      dto.presupuesto,
  ...
  @ApiOperation({
    ...
    description:
      'El cliente dueño ve los hasta 3 candidatos; un prestador candidato solo ve su propia postulación.',
  })
  async getCandidatos(
    @Param('id') id: string,
    @CurrentUser() user: User,
  ...
  @ApiOperation({
    summary: 'Elegir un candidato (solo el cliente dueño de la solicitud)',
  })
  @ApiResponse({ status: 200 })
  @ApiResponse({
    status: 409,
    description: 'Candidato ya no disponible o ya se eligió uno',
  })
  async elegirCandidato(
    ...
    this.logger.log(
      `Elegir candidato ${candidatoId} en solicitud ${id} - cliente: ${user.id}`,
    );
    return this.solicitudesService.elegirCandidato(
      id,
      candidatoId,
      user.id,
      accessToken,
  ...
  @ApiOperation({
    summary:
      'Cancelar solicitud (cliente en buscando/aceptado, prestador solo en aceptado)',
    description:
  ...
  @ApiResponse({
    status: 409,
    description: 'El estado cambió justo antes de poder cancelarla',
  })
  async cancel(
    @Param('id') id: string,
    @CurrentUser() user: User,
  ...
  @ApiOperation({
    ...
    description:
      'aceptado→finalizado (prestador) | finalizado→cerrado (cliente)',
  })
  ...
  async updateStatus(
    ...
    this.logger.log(
      `Update status solicitud ${id} → ${dto.estado} - user: ${user.id}`,
    );
